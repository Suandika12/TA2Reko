...
import 'package:recomend_toba/data_wilayah/data_wilayah_tampil.dart';
import 'package:recomend_toba/data_wilayah/data_wilayah_tampil_select.dart';
...
class DataWilayahScreen extends StatefulWidget {
  static const routeName = '/data_wilayah';

  const DataWilayahScreen({super.key});

  @override
  State<DataWilayahScreen> createState() => _DataWilayahScreenState();
}

class _DataWilayahScreenState extends State<DataWilayahScreen> {
  ...
  String valuePencarian = "id_wilayah";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: MultiBlocListener(
        listeners: [
          BlocListener<DataWilayahHapusBloc, DataWilayahHapusState>(
            listener: (context, state) {
              if (state is DataWilayahHapusLoadSuccess) {
                fetchData();
              }
            },
          ),
          BlocListener<DataWilayahUbahBloc, DataWilayahUbahState>(
            listener: (context, state) {
          ...
          BlocListener<DataWilayahSimpanBloc, DataWilayahSimpanState>(
            listener: (context, state) {
                  ...
                  children: [
                    ...
                    SingleChildScrollView(
                      ...
                      child: Row(
                        children: [
                          ...
                          //       context,
                          //       DataWilayahTambahScreen.routeName,
                          //       arguments: DataWilayahTambahArguments(
                          //         data: DataWilayah(),
                          //         judul: "Tambah Data Wilayah",
                          //       ),
                          //     );
                          //     fetchData();
                          //   },
                    ...
                    Expanded(
                      child: Builder(builder: (context) {
                        final stateData =
                            context.watch<DataWilayahBloc>().state;
                        final stateHapus =
                            context.watch<DataWilayahHapusBloc>().state;
                        if (stateData is DataWilayahLoading ||
                            stateHapus is DataWilayahHapusLoading) {
                          return const LoadingWidget();
                        }
                        if (stateData is DataWilayahLoadSuccess) {
                          List<DataWilayahApiData> data = stateData.data.result;
                          if (data.isEmpty) {
                          ...
                          return ListView.builder(
                            ...
                            itemBuilder: ((context, index) {
                              return DataWilayahTampilSelect(
                                data: data[index],
                              );
                            }),
                          );
                        }
                        if (stateData is DataWilayahLoadFailure) {
  ...
  void fetchData() async {
    BlocProvider.of<DataWilayahBloc>(context).add(
      FetchDataWilayah(filter),
    );
  }

  void prosesHapus(value) {
    BlocProvider.of<DataWilayahHapusBloc>(
      context,
    ).add(
      FetchDataWilayahHapus(
        data: value,
      ),
    );
  }
